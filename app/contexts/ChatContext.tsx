...
interface ChatContextType {
  ...
  addEvent: (chatId: string, event: { id: string; title: string; date: string }) => Promise<void>;
  reload: () => void;
}
...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addEvent = async (chatId: string, event: { id: string; title: string; date: string }) => {
    const updated = chats.map((c) =>
      c.id === chatId ? { ...c, scheduledEvents: [...c.scheduledEvents, event] } : c
    );
    await saveChats(updated);
  };
  ...
  return (
    <ChatContext.Provider value={{ chats, sendMessage, updateRecruitmentStatus, addChat, addEvent, reload }}>
      {children}
