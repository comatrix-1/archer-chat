                ...
                <div className="flex-1">
                    ...
                    <MonthYearPicker
                        ...
                        onSelect={(date) => {
                            ...
                            setValue(`projects.${index}.startDate`, newDate, { shouldValidate: true, shouldDirty: true });
                        }}
                    />
                </div>
                <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-2">
                            <Checkbox
                                ...
                                onCheckedChange={(checked) => {
                                    setValue(`projects.${index}.endDate`, checked ? null : new Date(), { shouldValidate: true, shouldDirty: true });
                                }}
                            />
                            <label htmlFor={`project-present-${index}`} className="text-sm font-medium">Present</label>
                        </div>
                    </div>
                    {!isPresent && (
                        <MonthYearPicker
                            ...
                            onSelect={(date) => {
                                const newDate = date ? new Date(date) : null;
                                if (newDate) newDate.setHours(0, 0, 0, 0);
                                setValue(`projects.${index}.endDate`, newDate, { shouldValidate: true, shouldDirty: true });
                            }}
                        />
                    )}
                </div>
            ...
            <FormField
                ...
                render={({ field }) => (
                    <FormItem>
                        ...
                        <FormControl>
                            <RichTextEditor content={field.value} onChange={field.onChange} placeholder="Describe the project..." />
                        </FormControl>
                        <FormMessage />
                    </FormItem>
                )}
...
const ProjectSection: React.FC<ProjectSectionProps> = ({ projectFields, control, getValues, setValue, removeProject }) => {
