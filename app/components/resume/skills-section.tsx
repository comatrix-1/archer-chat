...
// Define categories based on the Harvard template example
const SKILL_CATEGORIES = ["Technical", "Language", "Laboratory", "Interests", "Other"];
...
const SkillsSection: React.FC<SkillsSectionProps> = ({ skills, control, removeSkill }) => {
    ...
    const groupedSkills = useMemo(() => {
        return skills.reduce((acc, skill, index) => {
            const category = skill.category || "Other"; // Default to "Other" if somehow empty
            if (!acc[category]) {
                acc[category] = [];
            }
        ...
        <div className="space-y-4">
            ...
            {Object.entries(groupedSkills).map(([category, categorySkills]) => (
                <div key={category} className="space-y-3 border-l-2 pl-4 border-muted">
                    <h3 className="text-md font-semibold text-muted-foreground">{category}</h3>
                        ...
                        >
                            ...
                            <div className="w-1/4">
                                <FormField
                                    ...
                                    render={({ field: renderField }) => ( // Renamed to avoid conflict
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                ...
                                                >
                                                    ...
                                                    <SelectContent>
                                                        ...
                                                        <SelectItem value="N/A">N/A</SelectItem> {/* Added N/A for things like Interests */}
                                                    </SelectContent>
                                                </Select>
                                            </FormControl>
                                            <FormMessage />
                            ...
                            {/* Remove Button */}
                            <Button type="button" variant="destructive" onClick={() => removeSkill(field.originalIndex)}>
                                <Trash2 size={16} />
                            </Button>
                        </div>
                    ))}
