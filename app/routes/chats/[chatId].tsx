...
const statuses = ["Pending", "Screening", "Interview", "Offer", "Rejected", "Hired"];
    ...
    <div className="max-w-2xl mx-auto p-4">
      ...
      <div className="bg-gray-100 rounded p-4 h-96 overflow-y-auto flex flex-col gap-2">
        {chat.messages.map(msg => (
          ...
          >
            <div className={`rounded px-3 py-2 max-w-xs ${msg.senderId === user?.id ? "bg-blue-500 text-white" : "bg-white border"}`}>
              <span>{msg.content}</span>
            </div>
            <span className="text-xs text-gray-500 mt-1">
              {msg.senderId === user?.id ? "You" : msg.senderName} · {format(new Date(msg.timestamp), "HH:mm, MMM d")}
            </span>
          </div>
        ))}
