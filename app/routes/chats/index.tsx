    ...
    <div className="max-w-2xl mx-auto p-4">
      ...
      <ul className="space-y-2">
        {chats.map(chat => (
          <li key={chat.id}>
            ...
            >
              <span>{chat.name}</span>
              <span className="text-xs bg-gray-200 px-2 py-1 rounded ml-2">{chat.recruitmentStatus}</span>
            </button>
          </li>
        ))}
      </ul>
      {open && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <form onSubmit={handleCreate} className="bg-white p-6 rounded shadow w-full max-w-sm">
