...
export function useResumeForm(initialResume: Resume & {
  contact: Contact;
  ...
  const form = useForm<ResumeFormData>({
    defaultValues: {
      ...
      licenseCertifications:
        initialResume.licenseCertifications?.map((cert: LicenseCertification) => ({
          ...cert,
  ...
  const onSubmit = async (data: ResumeFormData) => {
    ...
    try {
      ...
      if (responseData.resume) {
        ...
        setLoadingState(prev => ({ ...prev, isSubmitting: false, error: errorMsg }));
